...
/**
 * 此类测试初始化、异常注册、实时预览（对象方式），没有对象，在成员变量上已经写死了ip
 * <p>
 * 1/还有内部有些问题，画面卡顿，没有解决。
 * <p>
 * 2/没有解决inputStream流的睡眠问题。
 */
...
//        public int dwStreamType;码流类型：0-主码流，1-子码流，2-码流3，3-虚拟码流
    ...
    public void processRealData(int iDataType,
        ...
        if (!m_bNeedDecode) {
            ...
            if (hCNetSDK.NET_DVR_SYSHEAD == iDataType) {
                ...
                m_iPort = player.getPort();//获取播放库未使用的通道号0
                if (m_iPort == -1) {
                    LogUtils.logE(TAG, "PlayM4_GetLastError-getPort is failed with: "
                            + player.getLastError(m_iPort));
                    return;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.row_1_sv_1:
               /* //未登陆
                if (m_iLogID < 0) {
                    AlertDialog.Builder builder = new AlertDialog.Builder(Test3Activity.this);
                    View view = LayoutInflater.from(Test3Activity.this).inflate(R.layout.layout_dialog, null);
                    ...
                    btn_Login.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            // TODO: 2019/4/8
                            ToastUtil.showMsg2(Test3Activity.this, "登陆了");
                            alertDialog.dismiss();
                        }
                    });
                } else {
                    CustomDialog customDialog = new CustomDialog(Test3Activity.this, R.style.CustomDialog);
                    customDialog.setTitle("提示").setMessage("是否登出？")
                            .setCancel("取消", new CustomDialog.IOnCancelListener() {
                                @Override
                                public void onCancel(CustomDialog customDialog) {
                                    ToastUtil.showMsg2(Test3Activity.this, "Cancel……");
                                    customDialog.dismiss();
                                }
                            }).setConfirm("确认", new CustomDialog.IOnConfirmListener() {
                        @Override
                        public void onConfirm(CustomDialog customDialog) {
                            ToastUtil.showMsg2(Test3Activity.this, "Confirm……");
                            customDialog.dismiss();
                        }
                    }).show();
...
//            holder.setFormat(PixelFormat.TRANSLUCENT);
            ...
            LogUtils.logD(TAG, "surface is created，播放库未使用的通道号:" + m_iPort);
            if (-1 == m_iPort) {
                return;
            }
            //Surface通过SurfaceHolder接口中的getSurface()获取
    ...
    private void riskAuthorization() {
        ...
        */
        ...
        //检查是否获得了权限
        if (ContextCompat.checkSelfPermission(Test3Activity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
        }
        if (ContextCompat.checkSelfPermission(Test3Activity.this, Manifest.permission.CAPTURE_AUDIO_OUTPUT) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.CAPTURE_AUDIO_OUTPUT);
        }
        if (ContextCompat.checkSelfPermission(Test3Activity.this, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.RECORD_AUDIO);
        }
        if (!permissionList.isEmpty()) {
            String[] permissions = permissionList.toArray(new String[permissionList.size()]);
            ActivityCompat.requestPermissions(Test3Activity.this, permissions, 1);
            //申请权限
            //开始弹出授权信息，并决定是否同意，之后会调onRequestPermissionsResult()回调
        } else {
            // TODO: 2019/4/17
            ToastUtil.showMsg2(Test3Activity.this, "授权成功了");
        }
    }
