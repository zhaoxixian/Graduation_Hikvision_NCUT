...
/**
 * 在test9activity的基础上，试解决，在判断不合法时，对话框关闭，return整个方法了
 * <p>
 * 还有开始合并，在每个surfaceview上都实现同样的效果，因为之前都是在一个surfaceview做实验
 *
 *
 * 意外收获，两个surfaceview能同时登陆了。。所以此实验类，作为一个里程碑
 */
public class Test10Activity extends BaseActivity implements View.OnClickListener {
        ...
        switch (v.getId()) {
            case R.id.row_1_sv_1:
                ...
                if (isLogin_row_1_col_1 < 0) {//isLogin_row_1_col_1=-1，未登陆，或者登陆失败
                    ...
                    //未登陆
                    final AlertDialog.Builder builder = new AlertDialog.Builder(Test10Activity.this);
                    builder.setTitle("请先登陆");

                    //通过LayoutInflater来加载一个xml的布局文件作为一个View对象
                    View view = LayoutInflater.from(Test10Activity.this).inflate(R.layout.layout_dialog, null);
...
//                                return;
                            ...
                            ToastUtil.showMsg2(Test10Activity.this, "登陆成功……");
                            dialog.dismiss();
                        }
                    });
...
//                    Button btn_Login = (Button) view.findViewById(R.id.btn_login);
                    ...
                    builder.setNegativeButton("取消登陆", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            ToastUtil.showMsg2(Test10Activity.this, "取消登陆……");
                            dialog.dismiss();
                        }
                    });
                    ...
                    CustomDialog customDialog = new CustomDialog(Test10Activity.this, R.style.CustomDialog);
                    customDialog.setTitle("提示").setMessage("是否登出 ?")
                            .setCancel("取消", new CustomDialog.IOnCancelListener() {
                                @Override
                                public void onCancel(CustomDialog customDialog) {
                                    ToastUtil.showMsg2(Test10Activity.this, "Cancel……");
                        ...
                        public void onConfirm(CustomDialog customDialog) {
                           ...
                           /* if (!onlyOneIp.isEmpty()) {
                            ...
                            ToastUtil.showMsg2(Test10Activity.this, "登出成功啦!");
...
//                    Button btn_Login = (Button) view.findViewById(R.id.btn_login);
    ...
    private void riskAuthorization() {
        ...
        */
        ...
        //检查是否获得了权限
        if (ContextCompat.checkSelfPermission(Test10Activity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
        }
        if (ContextCompat.checkSelfPermission(Test10Activity.this, Manifest.permission.CAPTURE_AUDIO_OUTPUT) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.CAPTURE_AUDIO_OUTPUT);
        }
        if (ContextCompat.checkSelfPermission(Test10Activity.this, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.RECORD_AUDIO);
        }
        if (!permissionList.isEmpty()) {
            String[] permissions = permissionList.toArray(new String[permissionList.size()]);
            ActivityCompat.requestPermissions(Test10Activity.this, permissions, 1);
            //申请权限
            //开始弹出授权信息，并决定是否同意，之后会调onRequestPermissionsResult()回调
        } else {
            // TODO: 2019/4/17
            ToastUtil.showMsg2(Test10Activity.this, "授权成功了");
        }
    }
...
//        super.onBackPressed();
        ...
        if (lastClickTime <= 0) {
            ...
            if (currentClickTime - lastClickTime < 1000) {
                ...
                ToastUtil.showMsg2(Test10Activity.this, "资源已回收,再会了!");
